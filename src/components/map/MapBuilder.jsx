...
export default function MapBuilder({ terrain }) {
  ...
  const [colorRev, setColorRev] = useState(0) // bump → scene recolour
  ...
  const paintCell = useCallback(
    (cx, cy) => {
      ...
      for (const i of cells) {
        if (stroke.has(i)) continue // one step per cell per stroke (predictable)
  ...
  const endStroke = useCallback(() => {
    ...
    const entries = []
    for (const [i, before] of stroke) {
      const after = heightsRef.current[i]
  ...
  const undo = useCallback(() => {
    const entries = undoRef.current.pop()
    if (!entries) return
    for (const { i, before } of entries) heightsRef.current[i] = before
  ...
  const redo = useCallback(() => {
    const entries = redoRef.current.pop()
    if (!entries) return
    for (const { i, after } of entries) heightsRef.current[i] = after
    undoRef.current.push(entries)
